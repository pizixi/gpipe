...
import { normalizeListenAddress } from '../utils/helpers';
...
const AddTunnelModal: React.FC<Props> = ({ open, onClose, onSuccess }) => {
  ...
  const handleOk = async () => {
    ...
    setLoading(true);
    try {
      const res = await tunnelsApi.addTunnel({
      ...
      if (res.code === 0) {
        ...
        message.error(t('add_tunnel_failed') + res.msg);
      }
    } catch {
      // ignore
    }
